...
// function as ProcessSnapshot mutator.
type ProcessSnapshotFunc func(context.Context, *ent.ProcessSnapshotMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ProcessSnapshotFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProcessSnapshotMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProcessSnapshotMutation", m)
}
