...
// QueryProcessSnapshots chains the current query on the "process_snapshots" edge.
func (cq *CollectionQuery) QueryProcessSnapshots() *ProcessSnapshotQuery {
...
func (cq *CollectionQuery) Clone() *CollectionQuery {
	...
	return &CollectionQuery{
		...
		withProcessSnapshots: cq.withProcessSnapshots.Clone(),
...
// WithProcessSnapshots tells the query-builder to eager-load the nodes that are connected to
// the "process_snapshots" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CollectionQuery) WithProcessSnapshots(opts ...func(*ProcessSnapshotQuery)) *CollectionQuery {
	query := (&ProcessSnapshotClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withProcessSnapshots = query
	return cq
}
...
func (cq *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := cq.withProcessSnapshots; query != nil {
		if err := cq.loadProcessSnapshots(ctx, query, nodes,
			func(n *Collection) { n.Edges.ProcessSnapshots = []*ProcessSnapshot{} },
...
func (cq *CollectionQuery) loadProcessSnapshots(ctx context.Context, query *ProcessSnapshotQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *ProcessSnapshot)) error {
	...
	query.withFKs = true
	query.Where(predicate.ProcessSnapshot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.ProcessSnapshotsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.collection_process_snapshots
		if fk == nil {
			return fmt.Errorf(`foreign-key "collection_process_snapshots" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "collection_process_snapshots" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
