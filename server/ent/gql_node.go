...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case frameinfo.Table:
		...
		return n, nil
	case processsnapshot.Table:
		query := c.ProcessSnapshot.Query().
			Where(processsnapshot.ID(id))
		query, err := query.CollectFields(ctx, processsnapshotImplementors...)
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case processsnapshot.Table:
		query := c.ProcessSnapshot.Query().
			Where(processsnapshot.IDIn(ids...))
		query, err := query.CollectFields(ctx, processsnapshotImplementors...)
		if err != nil {
			return nil, err
		}
